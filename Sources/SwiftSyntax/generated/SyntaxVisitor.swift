...
open class SyntaxVisitor {
  ...
  private func visitImpl2(
    _ node: Syntax,
    _ nodeType: SyntaxProtocol.Type,
    _ visit: (any SyntaxProtocol) -> SyntaxVisitorContinueKind,
    _ visitPost: (any SyntaxProtocol) -> Void
  ) {
    let node = node.cast(nodeType.self)
    let needsChildren = (visit(node) == .visitChildren)
    // Avoid calling into visitChildren if possible.
...
//    case .accessorBlock:
//      return {
        self.visitImpl(node, AccessorBlockSyntax.self, self.visit, self.visitPost)
//      }
